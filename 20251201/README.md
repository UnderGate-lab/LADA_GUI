# lada簡易GUI解説（公式 0.8.1 以降の LADA Windows版パッケージに対応）
...
pip install opencv-python pillow tkinterdnd2 numpy psutil
```

※lada-cli.exeを外部プログラムとして呼び出すだけなのでどのpython環境で動作させても問題ありません。    
`python -m pip install <Package>`により、実行したpythonコマンドのpython環境にパッケージがインストールされます。

### （２）ffmpeg.exeのコピー

LADAインストールフォルダ（lada-cli.exeがあるフォルダ）の`\_internal\bin`の下にある`ffmpeg.exe`を、  
LADAインストールフォルダ直下にコピー（※移動ではない）してください。  
これは動画切り出しに使用するためです。  
環境変数にffmpeg.exeのパスが通っている場合はこの作業は不要です。

### （３）プログラムのコピー

`lada_gui.py`をLADAインストールフォルダにコピーします。 

## ４．実行方法

LADAインストールフォルダから以下のコマンドを実行
...
再エンコード（hevc_nvenc）にするとエラーがでないと思います。  
