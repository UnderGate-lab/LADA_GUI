...
class MosaicRemoverApp:
    def __init__(self, root):
        ...
        # lada-cli.exeのパスを設定
        #self.lada_cli_path = os.path.join(self.script_dir, "lada-cli")
        self.lada_cli_path = "/home/radiocorner/lada/.venv/bin/lada-cli"
    ...
    def apply_vr_undistortion(self, input_file, output_file, unique_id):
        ...
        subprocess.run(crop_center_cmd, check=True)
    ...
    def run_lada_cli(self, input_file, unique_id):
        ...
        output_file = os.path.join(self.output_dir, f"{input_basename}_lada.mp4")
        ...
        detect_model_path = os.path.join(self.script_dir, "model_weights", detect_model)
        
        # 復元モデル（固定）
        #restore_model_path = os.path.join(self.script_dir, "_internal", "model_weights", 
        #                                "lada_mosaic_restoration_model_generic_v1.2.pth")

        restore_model_path = os.path.join(self.script_dir, "model_weights", 
                                        "lada_mosaic_restoration_model_generic_v1.2.pth")
        ...
        # 出力をリアルタイムで表示（Processing frames: はログに記録しない）
        while True:
            line = process.stdout.readline()
            if not line and process.poll() is not None:
                break
            if line:
                # Processing frames: の行はログに記録しない
    ...
    def processing_main(self, input_file, start_time_sec, end_time_sec, vr_simple_mode=None):
        ...
        try:
            ...
            if is_vr_mode:
                ...
                # lada-cli.exeを直接実行
                center_processed = self.run_lada_cli(center_file, unique_id)
                ...
                # 通常の2D処理モード
                # lada-cli.exeを直接実行
                processed_file_path = self.run_lada_cli(trimmed_file_path, unique_id)
    ...
    def merge_vr_video(self, unique_id, parts, output_file, audio_file):
        ...
        # LADA処理済みの中央領域ファイルを探す（新しい命名規則）
        center_processed = None
        for file in os.listdir(self.output_dir):
            # 新しい命名規則: {unique_id}_center_lada.mp4
            if (f'{unique_id}_center' in file and 
                file.endswith('_lada.mp4')):
        ...
        if not center_processed or not os.path.exists(center_processed):
            ...
            for file in os.listdir(self.output_dir):
                if (f'{unique_id}_center' in file and 
                    ...
                    file.endswith('.mp4')):
                    center_processed = os.path.join(self.output_dir, file)
                    self.write_log(f"LADA処理済みファイル検出（旧）: {file}")
                    break
