...
class MosaicRemoverApp:
    def __init__(self, root):
        self.root = root
        self.root.title("LADA GUI 20260102-1 for lada 0.10.0")
        self.root.geometry("1120x1000")
        self.root.protocol("WM_DELETE_WINDOW", self.on_closing)
        
        # パスの取得方法を修正
        if getattr(sys, 'frozen', False):
            # EXEとして実行されている場合（EXE本体があるディレクトリ）
        ...
        self.model_weights_dir = os.path.join(internal_dir, "model_weights")
        self.ffmpeg_bin_dir = os.path.join(internal_dir, "bin") # ffmpeg.exe がある場所
        self.ffmpeg_path = os.path.join(self.ffmpeg_bin_dir, "ffmpeg.exe")

        # 重要: システムの PATH に ffmpeg の場所を追加しておく
        # これにより、lada-cli.exe が内部で "ffmpeg" とだけ呼んでも見つけられるようになります
        ...
        # lada-cli.exeの存在チェック
        if not os.path.exists(self.lada_cli_path):
            messagebox.showerror("エラー", "lada-cli.exe が見つかりません。")
            self.lada_cli_path = None
    ...
    def create_widgets(self):
        ...
        options_frame = tk.LabelFrame(main_frame, text="2. オプション設定", padx=10, pady=10)
        ...
        # エンコーディングプリセットの追加
        preset_label = tk.Label(options_frame, text="エンコーディングプリセット:")
        preset_label.pack(side=tk.LEFT, padx=(15, 5))
        self.preset_var = tk.StringVar(value=self.cli_options["encoding_preset"])
        self.preset_var.trace_add("write", self.save_config_callback)
        preset_options = [
            "hevc-nvidia-gpu-balanced",
            "hevc-nvidia-gpu-hq",
            "hevc-nvidia-gpu-uhq",
            "h264-nvidia-gpu-fast",
            "h264-cpu-fast",
            "h264-cpu-uhq",
            "av1-cpu-uhq"
        ]
        self.preset_menu = tk.OptionMenu(options_frame, self.preset_var, *preset_options)
        self.preset_menu.pack(side=tk.LEFT, padx=5)
        
        # 顔モザイク検出オプションの追加
        face_detection_label = tk.Label(options_frame, text="顔モザイク検出:")
        face_detection_label.pack(side=tk.LEFT, padx=(15, 5))
        self.face_detection_var = tk.StringVar(value=self.cli_options["face_detection"])
        self.face_detection_var.trace_add("write", self.save_config_callback)
        tk.Radiobutton(options_frame, text="有効", variable=self.face_detection_var, value="True").pack(side=tk.LEFT)
        tk.Radiobutton(options_frame, text="無効", variable=self.face_detection_var, value="False").pack(side=tk.LEFT, padx=(0, 5))
    ...
    def run_lada_cli(self, input_file, unique_id):
            ...
            if self.face_detection_var.get() == "True":
                face_detection_option = ["--detect-face-mosaics"]
            else:
                face_detection_option = ["--no-detect-face-mosaics"]
            
            # エンコーディングプリセットオプションの追加
            lada_command = [
                self.lada_cli_path,
                "--input", input_file,
                "--output", output_file_temp,
                "--encoding-preset", self.preset_var.get(),
                "--mosaic-detection-model", detect_model_path,
                "--mosaic-restoration-model", restore_model_path,
                "--device", "cuda:0",
                "--max-clip-length", "180"
            ] + face_detection_option
    ...
    def update_queue_listbox(self):
        ...
        for i, entry in enumerate(self.processing_queue):
            try:
                ...
                encoding_preset = entry.get('encoding_preset', 'hevc-nvidia-gpu-balanced')
                face_detection = face_detection_map.get(entry.get('face_detection', 'False'), '顔検出:無効')
                ...
                display_text = (f"{i+1}. {filename}, Model:{model_label}, Preset:{encoding_preset}, "
                               f"{face_detection}, {range_display}, FFmpeg:{ffmpeg_display}, CRF:{crf_value}, "
    ...
    def load_config(self):
            ...
            try:
                with open(self.config_file, 'r', encoding='utf-8') as f:
                    ...
                    for line in lines:
                        ...
                        if line.startswith("model="):
                        ...
                        elif line.startswith("encoding_preset="):
                            preset = line.split("=")[1]
                            valid_presets = [
                                "hevc-nvidia-gpu-balanced",
                                "hevc-nvidia-gpu-hq",
                            ...
                            if preset in valid_presets:
                                self.cli_options["encoding_preset"] = preset
                                self.preset_var.set(preset)
                        elif line.startswith("face_detection="):
                            face_detection = line.split("=")[1]
                            if face_detection in ["True", "False"]:
                                self.cli_options["face_detection"] = face_detection
                                self.face_detection_var.set(face_detection)
        ...
        # エンコーディングプリセットが空の場合はデフォルトを設定
        if not self.preset_var.get():
            self.preset_var.set(self.cli_options["encoding_preset"])

        # 顔モザイク検出が空の場合はデフォルトを設定
        if not self.face_detection_var.get():
            self.face_detection_var.set(self.cli_options["face_detection"])
    ...
    def save_config(self):
        try:
            with open(self.config_file, 'w', encoding='utf-8') as f:
                ...
                f.write(f"encoding_preset={self.preset_var.get()}\n")
                f.write(f"face_detection={self.face_detection_var.get()}\n")
    ...
    def batch_process_main(self):
        ...
        while self.processing_queue and self.is_batch_processing:
            ...
            try:
                ...
                # バッチ処理用に現在の設定を一時的に変更
                original_preset = self.preset_var.get()
                original_face_detection = self.face_detection_var.get()
                
                # キューエントリーの設定を適用
                self.preset_var.set(entry.get('encoding_preset', 'hevc-nvidia-gpu-balanced'))
                self.face_detection_var.set(entry.get('face_detection', 'False'))
                ...
                # 元の設定に戻す
                self.preset_var.set(original_preset)
                self.face_detection_var.set(original_face_detection)
    ...
    def processing_main(self, input_file, start_time_sec, end_time_sec, vr_simple_mode, ffmpeg_option):
        ...
        try:
            ...
            if is_vr_mode:
                ...
                cli_options_tag = f"model{self.model_var.get()}_preset{self.preset_var.get()}_face{self.face_detection_var.get()}"
    ...
    def open_log_file(self):
        try:
            ...
            messagebox.showerror("エラー", f"ログファイルを開けませんでした: {str(e)}")
